...
const {token} = useStateContext();
  const [hasImmeuble, setHasImmeuble] = useState(null);
  ...
  useEffect(() => {
    ...
    if(!token){
         navigate("/auth/loginT")
    }
    const checkImmeuble = async () => {
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setHasImmeuble(false);
        }
      } catch (err) {
        console.error('Error checking immeuble:', err);
